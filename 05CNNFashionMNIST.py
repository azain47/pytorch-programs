...
train_data = datasets.FashionMNIST(root='./datasets', train=True,transform=transforms.ToTensor(),
                                   download=True)

test_data = datasets.FashionMNIST(root='./datasets',train=False,transform=transforms.ToTensor(),
                                  download=True)
...
class CNN(nn.Module):
    def __init__(self, K) :
        ...
        self.dense_layers = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(in_features=128*2*2,out_features=800),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(800,K)
        )
